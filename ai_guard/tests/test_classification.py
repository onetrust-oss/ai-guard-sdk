...
class TestAIGuardClient(unittest.TestCase):
    ...
    def test_classify_success_returns_response(self):
        ...
        self.assertEqual(
            payload["context"],
            {"agent_id": "test-agent", "platform": AIPlatform.AMAZON_BEDROCK},
        )
...
class TestClassificationResponse(unittest.TestCase):
        ...
        response = ClassificationResponse.from_dict(
            {
                ...
                "matches": [
                    ...
                    {"start": 20, "end": 30, "confidence": 100, "classifier": "US_SSN"},
                    {
                        "start": 40,
                        "end": 55,
                        "confidence": 85,
    ...
    def test_repr_with_no_matches(self):
        response = ClassificationResponse.from_dict({"context": None, "matches": []})

        result = json.loads(f"{response}")
        expected = {"context": None, "matches": []}
        self.assertEqual(result, expected)
