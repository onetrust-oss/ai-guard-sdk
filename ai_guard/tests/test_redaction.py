...
class TestClassificationRedactor(unittest.TestCase):
    def test_redaction(self):
        ...
        redact = redactor.redact(
            text="phone 321-507-0525 number", classification=classification
        )
        ...
        self.assertTrue(
            all(action.kind != RedactKind.NONE for action in redact.actions)
        )
    ...
    def test_default_action_none_with_multiple_matches(self):
        ...
        self.assertEqual(
            redact.text, "phone              and john@email.com contact 123-45-6789"
        )
        self.assertEqual(
            redact.actions,
            [RedactAction(kind=RedactKind.REDACT, classifier="US_PHONE_NUMBER")],
    ...
    def test_default_action_block_with_multiple_matches(self):
        ...
        redact = redactor.redact(
            text="phone 321-507-0525 and john@email.com", classification=classification
        )

        self.assertEqual(redact.text, "")
        self.assertEqual(
            redact.actions,
            [RedactAction(kind=RedactKind.BLOCK, classifier="EMAIL_ADDRESS")],
        )
        self.assertTrue(
            all(action.kind != RedactKind.NONE for action in redact.actions)
        )
...
class TestRedaction(unittest.TestCase):
    ...
    def test_repr_with_single_action(self):
        redaction = Redaction(
            actions=[RedactAction(kind=RedactKind.REDACT, classifier="EMAIL_ADDRESS")],
            text="text",
        )

        result = json.loads(f"{redaction}")
        expected = {"actions": [{"kind": "REDACT", "classifier": "EMAIL_ADDRESS"}]}
        self.assertEqual(result, expected)

    def test_repr_with_no_actions(self):
        redaction = Redaction(actions=[], text="unchanged text")
