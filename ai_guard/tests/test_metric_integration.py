...
def poll_for_metric(
    service, meter_name: str, agent_id: str, timeout: float = POLL_TIMEOUT
) -> dict:
    import time
    ...
    delay = 0.1
    while True:
        metrics = service.poll_metrics(
            min_count=1, timeout=max(0.1, deadline - time.monotonic())
        )
...
class TestMetricIntegration(unittest.TestCase):
    ...
    def test_metric_agent_event(self):
        ...
        metric = poll_for_metric(
            self.server.classification_service,
            "ai_guard.agent",
            agent_id,
        )
